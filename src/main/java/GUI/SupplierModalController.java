...
public class SupplierModalController {
    ...
    private void insertSupplier() {
        ...
        if (isValidInput()) {
            ...
            int insertResult = supplierBus.insert(temp, SessionManagerService.getInstance().employeeRoleId(),
            ...
            switch (insertResult) {
                ...
                case 2 ->
                    NotificationUtils.showErrorAlert("Có lỗi khi thêm nhà cung cấp. Vui lòng thử lại.", "Thông báo");
                ...
                case 5 -> NotificationUtils.showErrorAlert("Thêm nhà cung cấp vào CSDL thất bại. Vui lòng thử lại.",
                        "Thông báo");
    ...
    private void updateSupplier() {
        ...
        if (isValidInput()) {
            SupplierDTO temp = new SupplierDTO(supplier.getId(), txtCompanyName.getText().trim(),
                    txtPhone.getText().trim(), txtAddress.getText().trim(), txtEmail.getText().trim(),
                    cbSelectStatus.getValue().equals("Hoạt động"));
            int updateResult = supplierBus.update(temp, SessionManagerService.getInstance().employeeRoleId(),
                    SessionManagerService.getInstance().employeeLoginId());
            switch (updateResult) {
                case 1 -> {
                    isSaved = true;
                ...
                case 2 -> NotificationUtils
                        .showErrorAlert("Có lỗi khi cập nhật thông tin nhà cung cấp. Vui lòng thử lại", "Lỗi");
                case 3 -> NotificationUtils.showErrorAlert("Thông tin nhà cung cấp bị trùng lặp.", "Lỗi");
                case 4 ->
                    NotificationUtils.showErrorAlert("Không có quyền cập nhật thông tin nhà cung cấp.", "Thông báo.");
                case 5 -> NotificationUtils
                        .showErrorAlert("Không thể cập nhật thông tin nhà cung cấp. Vui lòng thử lại.", "Lỗi");
