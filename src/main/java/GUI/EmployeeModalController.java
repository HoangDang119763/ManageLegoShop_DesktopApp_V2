...
public class EmployeeModalController {
    ...
    private boolean isValidInput() {
        ...
        if (isValid && lastName.isEmpty()) {
            ...
            NotificationUtils.showErrorAlert(
                    "Tên nhân viên không hợp lệ (Tối đa 100 ký tự, chỉ chữ và số, \"_\", \"-\", \"/\").", "Thông báo");
            clearAndFocus(txtLastName);
            isValid = false;
        }
        ...
        if (isValid && salary.isEmpty()) {
            ...
            try {
                ...
                if (!validator.validateSalary(salaryS, 10, 2, false)) {
                    NotificationUtils.showErrorAlert(
                            "Lương cơ bản không hợp lế (Tối đa 10 chữ số, 2 số thập phân, không âm hoặc bằng 0).",
                            "Thông báo");
                    clearAndFocus(txtSalary);
                    isValid = false;
                }
    ...
    private void insertEmployee() {
        ...
        if (isValidInput()) {
            EmployeeDTO temp = new EmployeeDTO(Integer.parseInt(txtEmployeeId.getText().trim()),
            ...
            int insertResult = emBus.insert(temp, SessionManagerService.getInstance().employeeRoleId(),
            ...
            switch (insertResult) {
                ...
                case 3 ->
                    NotificationUtils.showErrorAlert("Bạn không có quyền \"Thêm nhân viên\" để thực hiện thao tác này.",
                            "Thông báo");
    ...
    private void updateEmployee() {
        ...
        if (isValidInput()) {
            EmployeeDTO temp = new EmployeeDTO(employee.getId(), txtFirstName.getText().trim(),
                    ...
                    getSelectedRole(), cbSelectStatus.getValue().equals("Hoạt động"),
                    "", 0, null, false, false, false, false, false, false);
            int updateResult = emBus.update(temp, SessionManagerService.getInstance().employeeRoleId(),
                    SessionManagerService.getInstance().employeeLoginId());
            switch (updateResult) {
                case 1 -> {
                    isSaved = true;
                ...
                case 2 ->
                    NotificationUtils.showErrorAlert("Có lỗi khi cập nhật nhân viên. Vui lòng thử lại.", "Thông báo");
                case 3 ->
                    NotificationUtils.showErrorAlert(
                            "Bạn không có quyền \"Cập nhật nhân viên\" để thực hiện thao tác này.", "Thông báo");
                ...
                case 6 ->
                    NotificationUtils.showErrorAlert("Bạn không thể cập nhật nhân viên ngang quyền.", "Thông báo");
                case 7 ->
                    NotificationUtils.showErrorAlert("Cập nhật nhân viên thất bại. Vui lòng thử lại sau.", "Thông báo");
                default -> NotificationUtils.showErrorAlert("Lỗi không xác định. Vui lòng thử lại.", "Thông báo");
            }
        }
    }
