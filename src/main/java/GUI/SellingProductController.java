...
public class SellingProductController {
    ...
    private void loadProductWrapper() {
        ...
        addConstraintRow(gpShowProductWrapper, ProductBUS.getInstance().filterProducts("", "", -1, 1, null, null, true),
                140);
    ...
    private void addProductToGrid(GridPane gridPane, int row, int col, Image image, String name, String categoryName,
                ...
                if (selectedDiscount.getType() == 0) { // Giảm theo tỷ lệ phần trăm
                    ...
                    discountPrice = totalPriceInvoice.multiply(
                            (selectedDetailDiscountList.get(i).getDiscountAmount())
                                    .divide(BigDecimal.valueOf(100), 2, BigDecimal.ROUND_HALF_UP) // Sử dụng setScale và
                                                                                                  // ROUND_HALF_UP thay
                                                                                                  // cho ROUND_CEILING
                    );
    ...
    private void handleCategoryFilterChange() {
        ArrayList<ProductDTO> list = ProductBUS.getInstance().filterProducts("", "",
                categoryMap.getOrDefault(cbCategoryFilter.getValue(), -1), 1, null, null, true);
        clearGrid(gpShowProductWrapper);
        if (list.isEmpty()) {
            return;
        }
    ...
    private void handleInvoice() {
        ...
        for (TempDetailInvoiceDTO t : arrTempDetailInvoice) {
            DetailInvoiceDTO di = new DetailInvoiceDTO(t.getInvoiceId(), t.getProductId(), t.getQuantity(),
                    t.getPrice(), BigDecimal.ZERO, t.getTotalPrice());
            ProductDTO p = new ProductDTO(t.getProductId(), t.getName(), t.getQuantity(), t.getPrice(), true, null,
                    null, 0);
            list.add(di);
            listProduct.add(p);
        }
        // Kiem tra lai tranh truong hop ap dung giam dung moc thanh cong nhung sau do
        ...
        if (selectedDiscount != null && selectedDetailDiscount != null) {
            if (totalPriceInvoice.compareTo(selectedDetailDiscount.getTotalPriceInvoice()) < 0) {
                NotificationUtils.showErrorAlert("Giá trị đơn hàng của bạn không còn đủ để áp dụng khuyến mãi này.",
                        "Thông báo");
                selectedDiscount = null;
                selectedDetailDiscount = null;
                txtCodeDiscount.setText("");
            ...
            // trường hợp hóa đơn tăng lên sau khi áp khuyến mãi trước đó. phải hỏi để chọn
            // lại km để áp mốc cao hơn
            checkDiscount();
        }

        InvoiceDTO temp = new InvoiceDTO(Integer.parseInt(txtInvoiceId.getText().trim()), null,
        ...
        String extra = "Tổng tiền hóa đơn: " + validate.formatCurrency(totalPriceInvoice) + " Đ" +
        ...
        if (!submit)
            return;
        boolean result = InvoiceService.getInstance().createInvoiceWithDetailInvoice(temp, ses.employeeRoleId(), list,
                ses.employeeLoginId());
    ...
    private void checkDiscount() {
        ...
        for (int i = length - 1; i >= 0; i--) {
            ...
            if (totalPriceInvoice.compareTo(selectedDetailDiscountList.get(i).getTotalPriceInvoice()) >= 0
                ...
                NotificationUtils.showInfoAlert("Hóa đơn của bạn đã đạt đủ điều kiện cho mốc giảm giá tiếp theo.",
                        "Thông báo");
                if (selectedDiscount.getType() == 0) { // Giảm theo tỷ lệ phần trăm
                    // Tính toán giá sau khi giảm theo tỷ lệ phần trăm
                    discountPrice = totalPriceInvoice.multiply(
