...
public class SellingProductController {
    ...
    public void loadTable() {
        ...
        tlb_col_price.setCellValueFactory(
                cellData -> new SimpleStringProperty(validationUtils.formatCurrency(cellData.getValue().getPrice())));
    ...
    private void onMousedClickSearchProduct() {
        ...
        ArrayList<ProductDTO> finalFilter = list.stream().filter(product -> product.getStockQuantity() > 0)
    ...
    private void addProductToGrid(GridPane gridPane, int row, int col, Image image,
            String name, String categoryName, int quantity, BigDecimal price, String id) {
        // ImageView
