...
// private ArrayList<TempDetailInvoiceDTO> arrTempDetailInvoice = new
// ArrayList<>();
...
// private ArrayList<DetailDiscountDTO> selectedDetailDiscountList = new
// ArrayList<>();
...
// cellData -> new
// SimpleStringProperty(String.valueOf(cellData.getValue().getQuantity())));
// tlb_col_price.setCellValueFactory(
// cellData -> new
// SimpleStringProperty(validationUtils.formatCurrency(cellData.getValue().getPrice())));
...
// btnExitSellingForm.setOnMouseClicked(event ->
// onMouseClickedExitSellingForm());
...
// btnGetCusInfo.setOnMouseClicked(event ->
// onMouseClickedShowCustomerContainer());
...
// btnGetDiscount.setOnMouseClicked(event ->
// onMouseClickedShowDiscountContainer());
// }

// // search
// private void onMousedClickSearchProduct() {
// ArrayList<ProductDTO> list = ProductBUS.getInstance().filterProducts("Tên sản
// phẩm",
...
// return;
// }
// ArrayList<ProductDTO> finalFilter = list.stream().filter(product ->
...
// CusForSellingModalController modalController =
...
// (UiUtils.gI()).openStage("/GUI/NavigatePermission.fxml", "Danh sách chức
// năng");
...
// private void addConstraintRow(GridPane gridPane, ArrayList<ProductDTO>
// products, double height) {
...
// private void addProductToGrid(GridPane gridPane, int row, int col, Image
// image, String name, String categoryName,
// int quantity, BigDecimal price, String id) {
// // ImageView
...
// Label priceValue = new
// Label(ValidationUtils.getInstance().formatCurrency(price) + " VNĐ");
...
// label.setStyle("-fx-font-family: Arial; -fx-font-size: 14px; -fx-font-weight:
// bold;");
// }
...
// productBox.setStyle("-fx-background-color: #e0ffff; -fx-border-color:
// #ccc;");
...
// NotificationUtils.showErrorAlert("Vui lòng bấm nhập phiếu mới @_@!", "Thông
// báo");
// return;
// }
...
// SellingProductModalController modalController =
// UiUtils.gI().openStageWithController(
...
// if (modalController != null && modalController.isSaved()) {
// this.arrTempDetailInvoice.set(selectedTempDetailInvoice.getInvoiceId(),
// modalController.getTempDetailInvoice());
...
// NotificationUtils.showInfoAlert("Xóa sản phẩm khỏi đơn hàng thành công.",
// "Thông báo");
// }
...
// private void addEventClickForProduct(TableView<TempDetailInvoiceDTO>
// tableView, GridPane gridPane) {
...
// DiscountForSellingModalController modalController =
// UiUtils.gI().openStageWithController(
// "/GUI/DiscountForSellingModal.fxml",
// discountForSellingModalController ->
// discountForSellingModalController.setPrice(totalPriceInvoice),
...
// // Kiểm tra xem totalPriceInvoice có lớn hơn hoặc bằng mốc giảm giá hiện tại
// // không
// if
// (totalPriceInvoice.compareTo(selectedDetailDiscountList.get(i).getTotalPriceInvoice())
// >= 0) {
...
// .divide(BigDecimal.valueOf(100), 2, BigDecimal.ROUND_HALF_UP) // Sử dụng
// setScale và
// // ROUND_HALF_UP thay
// // cho ROUND_CEILING
// );
...
// selectedDetailDiscount = new
// DetailDiscountDTO(selectedDetailDiscountList.get(i));
// return; // Nếu chỉ muốn trả về sau khi tìm thấy một mốc phù hợp, thì vẫn giữ
// return
// }
// }
// }
...
// lbTotalInvoicePrice.setText(ValidationUtils.getInstance().formatCurrency(totalPriceInvoice)
// + " Đ");
// lbDiscountPrice.setText(validate.formatCurrency(discountPrice) + " Đ");
// lbFinalTotalInvoicePrice.setText(validate.formatCurrency(finalTotalPriceInvoice)
// + " Đ");
// }
...
// BigDecimal total =
// temp.getPrice().multiply(BigDecimal.valueOf(temp.getQuantity()));
...
// selectedTempDetailInvoice =
// tbvDetailInvoiceProduct.getSelectionModel().getSelectedItem();
...
// NotificationUtils.showErrorAlert("Vui lòng thêm ít nhất một sản phẩm.",
// "Thông báo");
// return;
// }
// if (isNotSelectedCustomer()) {
// NotificationUtils.showErrorAlert("Vui lòng chọn khách hàng để bán hàng.",
// "Thông báo");
// return;
// }
...
// DetailInvoiceDTO di = new DetailInvoiceDTO(t.getInvoiceId(),
// t.getProductId(), t.getQuantity(),
// t.getPrice(), BigDecimal.ZERO, t.getTotalPrice());
// ProductDTO p = new ProductDTO(t.getProductId(), t.getName(), t.getQuantity(),
// t.getPrice(), true, null,
// null, 0);
// list.add(di);
// listProduct.add(p);
// }
// // Kiem tra lai tranh truong hop ap dung giam dung moc thanh cong nhung sau
// do
// // xoa bot khien gia k con du ap dung
// if (selectedDiscount != null && selectedDetailDiscount != null) {
// if
// (totalPriceInvoice.compareTo(selectedDetailDiscount.getTotalPriceInvoice()) <
// 0) {
// NotificationUtils.showErrorAlert("Giá trị đơn hàng của bạn không còn đủ để áp
// dụng khuyến mãi này.",
// "Thông báo");
// selectedDiscount = null;
// selectedDetailDiscount = null;
...
// return;
// }
// // trường hợp hóa đơn tăng lên sau khi áp khuyến mãi trước đó. phải hỏi để
// chọn
// // lại km để áp mốc cao hơn
// checkDiscount();
// }

// InvoiceDTO temp = new
// InvoiceDTO(Integer.parseInt(txtInvoiceId.getText().trim()), null,
// Integer.parseInt(txtEmployeeId.getText().trim()),
// Integer.parseInt(txtCustomerId.getText().trim()),
// selectedDiscount == null ? null : txtCodeDiscount.getText().trim(),
// discountPrice != null ? discountPrice : BigDecimal.ZERO, totalPriceInvoice);
// ValidationUtils validate = ValidationUtils.getInstance();
// String discount = "\nGiảm giá: ";
// if (selectedDiscount != null && selectedDetailDiscount != null) {
...
// discount += " - " +
// validate.formatCurrency(selectedDetailDiscount.getDiscountAmount()) + "%";
// }
// }
// String extra = "Tổng tiền hóa đơn: " +
// validate.formatCurrency(totalPriceInvoice) + " Đ" +
// "\nMã khuyến mãi: " + (selectedDiscount == null ? "Không có" :
// txtCodeDiscount.getText().trim()) +
// discount +
// "\nThành tiền: " + validate.formatCurrency(finalTotalPriceInvoice) + " Đ";

// boolean submit = NotificationUtils.showConfirmAlert("Xác nhận phiếu bán",
// arrTempDetailInvoice, "Thông Báo",
// extra);
// // khong xac nhan khong nhap
// if (!submit)
// return;
// boolean result =
// InvoiceService.getInstance().createInvoiceWithDetailInvoice(temp,
// ses.employeeRoleId(), list,
// ses.employeeLoginId());
// // Sau đó tăng số lượng sản phẩm và set lại giá
// if (result &&
// ProductBUS.getInstance().updateQuantitySellingPriceListProduct(listProduct,
// false)) {
...
// int length = selectedDetailDiscountList.size();
// for (int i = length - 1; i >= 0; i--) {
// // Kiểm tra xem totalPriceInvoice có lớn hơn hoặc bằng mốc giảm giá cao nhat
// // không
// if
// (totalPriceInvoice.compareTo(selectedDetailDiscountList.get(i).getTotalPriceInvoice())
// >= 0
// && selectedDetailDiscountList.get(i).getTotalPriceInvoice()
// .compareTo(selectedDetailDiscount.getTotalPriceInvoice()) > 0) {
// NotificationUtils.showInfoAlert("Hóa đơn của bạn đã đạt đủ điều kiện cho mốc
// giảm giá tiếp theo.",
// "Thông báo");
// if (selectedDiscount.getType() == 0) { // Giảm theo tỷ lệ phần trăm
// // Tính toán giá sau khi giảm theo tỷ lệ phần trăm
...
// DetailDiscountDTO(selectedDetailDiscountList.get(i));
// loadCaculatedTotalInvoicePrice();
// NotificationUtils.showInfoAlert("Áp dụng mốc khuyến mãi mới thành công.",
// "Thông báo");
// return;
// }
// }
