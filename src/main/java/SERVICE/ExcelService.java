...
// dataRow.createCell(6).setCellValue(role != null ?
// validate.formatCurrency(employee.getSalary().multiply(role.getSalaryCoefficient()))
// : validate.formatCurrency(employee.getSalary()));
...
// tồn tại.", "Thông báo");
// case 6 -> NotificationUtils.showErrorAlert("Có lỗi khi thêm danh sách sản
// phẩm qua Excel.", "Thông báo");
...
// là số).", ++errorCount)) break;
// continue;
// }
// if (categoryId < 0 ||
// !AvailableUtils.getInstance().isValidCategory(categoryId)) {
// if (handleError(errorMessages, row.getRowNum(), "Thể loại không hợp lệ hoặc
// đã bị xóa.", ++errorCount)) break;
// continue;
// }
...
// private boolean handleError(StringBuilder errorMessages, int rowNum, String
// message, int errorCount) {
// errorMessages.append("Dòng ").append(rowNum + 1).append(":
// ").append(message).append("\n");
// return errorCount >= 20;
// }

// public void
// exportToFileExcelProductRevenues(ArrayList<StatisticDTO.ProductRevenue>
// productRevenuesList, String timestamp, LocalDate start, LocalDate end) throws
// IOException {
// Workbook workbook = new XSSFWorkbook();
...
// public void
// exportToFileExcelEmployeeRevenues(ArrayList<StatisticDTO.QuarterlyEmployeeRevenue>
// employeeRevenueList, String timestamp, String year) throws IOException {
// Workbook workbook = new XSSFWorkbook();
// Sheet sheet = workbook.createSheet("TK_Employee_"+now());

// // Thêm tên bảng thống kê và thời gian từ ngày đến ngày
// Row headerInfoRow = sheet.createRow(0);
// headerInfoRow.createCell(0).setCellValue("Bảng thống kê doanh thu sản phẩm
// theo Quý năm " + year);
