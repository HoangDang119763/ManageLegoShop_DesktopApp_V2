...
public class AuthorizationService {
    ...
    public boolean hasPermission(int employeeId, int roleId, int permissionId) {
        if (employeeId <= 0 || roleId <= 0 || permissionId <= 0)
            return false;
    ...
    public boolean hasPermission(int employeeId, int roleId, PermissionKey permissionKey) {
        if (employeeId <= 0 || roleId <= 0 || permissionKey == null)
            return false;
        // Kiểm tra employeeId đó có role đó thật không và role có tồn tại không và hợp
        // lệ không
        if (isInvalidUserRole(employeeId, roleId) || !RoleBUS.getInstance().isValidRole(roleId))
        ...
        if (RolePermissionBUS.getInstance().isLocalEmpty())
            RolePermissionBUS.getInstance().loadLocal();
        PermissionDTO permission = PermissionBUS.getInstance().getByPermissionKeyLocal(permissionKey.name());
        // Kiểm tra role có quyền cụ thể không
        return RolePermissionBUS.getInstance().hasPermission(roleId, permission.getId());
    }
    ...
    public boolean isInvalidUserRole(int employeeId, int expectedRoleId) {
        if (employeeId <= 0 || expectedRoleId <= 0)
            return true;
        if (EmployeeBUS.getInstance().isLocalEmpty())
            EmployeeBUS.getInstance().loadLocal();
