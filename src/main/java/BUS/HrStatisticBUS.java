...
public class HrStatisticBUS {
    ...
    public HrStatisticDTO getHrStatistic(int month, int year) {
        ...
        List<HeadcountPoint> headcountOverTime =
                EmploymentHistoryDAL.getInstance().getHeadcountOverTime(month, year);
        List<StatusDistributionItem> statusDistribution =
                EmployeeDAL.getInstance().getStatusDistribution();
        List<DepartmentDistributionItem> departmentDistribution =
                EmployeeDAL.getInstance().getDepartmentDistribution();
    ...
    private RewardFineSummary buildRewardFineSummary(int month, int year) {
        LocalDate salaryPeriod = LocalDate.of(year, month, 1);

        ArrayList<AllowanceDTO> allowances = AllowanceBUS.getInstance().getBySalaryPeriod(salaryPeriod);
        BigDecimal totalAllowance = allowances.stream()
                .map(AllowanceDTO::getTotalAllowance)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        long employeesWithAllowance = allowances.stream()
                .map(AllowanceDTO::getEmployeeId)
                .distinct()
                .count();
